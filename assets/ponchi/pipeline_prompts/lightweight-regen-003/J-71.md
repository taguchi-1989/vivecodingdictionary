...
scene_goal: make RAM understandable as a simple book diagram
main_symbols: A system-memory workspace diagram: applications, data buffers, and cached files occupy general memory slots while the processor requests blocks.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for J-71 RAM.

Subject: The entry subject is RAM, but the visual subject is A system-memory workspace diagram: applications, data buffers, and cached files occupy general memory slots while the processor requests blocks.. Show Show generic motherboard memory modules and data movement; no brand logos, no readable part numbers. only as supporting elements.
