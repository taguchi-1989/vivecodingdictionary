...
scene_goal: make RTX シリーズ understandable as a simple book diagram
main_symbols: A consumer GPU series comparison: several generic graphics cards scale from small to large with rendering, AI, and video workload cards attached.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for J-74 RTX シリーズ.

Subject: The entry subject is RTX シリーズ, but the visual subject is A consumer GPU series comparison: several generic graphics cards scale from small to large with rendering, AI, and video workload cards attached.. Show Show hardware lineup as neutral cards; no NVIDIA or RTX logo, no product badge, no readable model numbers. only as supporting elements.
