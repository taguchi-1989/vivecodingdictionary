...
scene_goal: make 評価日・時変情報の見方 understandable as a simple book diagram
main_symbols: A time-sensitive information guide: an evaluation-date card connects to freshness indicators, update arrows, and caution markers.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for A-6 評価日・時変情報の見方.

Subject: The entry subject is 評価日・時変情報の見方, but the visual subject is A time-sensitive information guide: an evaluation-date card connects to freshness indicators, update arrows, and caution markers.. Show Show changing snapshots and verification loop; no readable dates, no calendar text, no product UI. only as supporting elements.
