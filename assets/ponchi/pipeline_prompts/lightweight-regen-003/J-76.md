...
scene_goal: make CPU understandable as a simple book diagram
main_symbols: A central-processor coordination map: a generic processor schedules tasks, talks to memory, storage, and accelerators, and manages control flow.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for J-76 CPU.

Subject: The entry subject is CPU, but the visual subject is A central-processor coordination map: a generic processor schedules tasks, talks to memory, storage, and accelerators, and manages control flow.. Show Use abstract hardware blocks and bus arrows; no Intel, AMD, or Arm logos; no readable instruction text. only as supporting elements.
