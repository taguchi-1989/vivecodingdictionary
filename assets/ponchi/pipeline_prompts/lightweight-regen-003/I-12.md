...
scene_goal: make Git MCP understandable as a simple book diagram
main_symbols: A local version-control connector: status, diff, branch, and commit cards pass through a server gate into a local repository graph.
...
Subject: The entry subject is Git MCP, but the visual subject is A local version-control connector: status, diff, branch, and commit cards pass through a server gate into a local repository graph.. Show Show abstract branch lines, staged cards, and result arrows; no Git logo, no GitHub logo, no branch names, no command text. only as supporting elements.
