...
scene_goal: make merge understandable as a simple book diagram
main_symbols: A merge concept diagram: two branch lanes converge through a merge gate into a single combined history lane.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for F-55 merge.

Subject: The entry subject is merge, but the visual subject is A merge concept diagram: two branch lanes converge through a merge gate into a single combined history lane.. Show A developer resolves two streams of work into one shared line; no Git logo, GitHub logo, product UI, or readable labels. only as supporting elements.
