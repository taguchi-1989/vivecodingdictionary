...
scene_goal: make ripgrep (rg) understandable as a simple book diagram
main_symbols: A fast code search workflow: query card scans folders and files, filters matching lines, and returns grouped results to a developer.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for F-71 ripgrep (rg).

Subject: The entry subject is ripgrep (rg), but the visual subject is A fast code search workflow: query card scans folders and files, filters matching lines, and returns grouped results to a developer.. Show A developer searches a project tree quickly; no ripgrep logo, product UI, or readable search text. only as supporting elements.
