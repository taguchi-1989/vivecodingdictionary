...
scene_goal: make .gitignore understandable as a simple book diagram
main_symbols: A file-ignore workflow: project files pass through a filter card that separates tracked files from ignored cache and build artifacts.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for F-56 .gitignore.

Subject: The entry subject is .gitignore, but the visual subject is A file-ignore workflow: project files pass through a filter card that separates tracked files from ignored cache and build artifacts.. Show A developer keeps generated files out of version tracking; no Git logo, GitHub logo, product UI, or readable filenames. only as supporting elements.
