...
scene_goal: make 拡散モデル understandable as a simple book diagram
main_symbols: A diffusion-generation process: noisy abstract pixels gradually denoise through repeated steps into a clean image-like composition card.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for J-23 拡散モデル.

Subject: The entry subject is 拡散モデル, but the visual subject is A diffusion-generation process: noisy abstract pixels gradually denoise through repeated steps into a clean image-like composition card.. Show Show a sequence of panels from noise to structure, with a guidance signal line; no real artwork copy, no readable prompt text, no product UI. only as supporting elements.
