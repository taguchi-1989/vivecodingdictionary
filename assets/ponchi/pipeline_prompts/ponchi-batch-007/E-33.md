...
scene_goal: make AgentBench understandable as a simple book diagram
main_symbols: An agent-suite benchmark map: multiple environment cards, tool-use lanes, memory block, and scoring dashboard surround a central agent evaluator.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for E-33 AgentBench.

Subject: The entry subject is AgentBench, but the visual subject is An agent-suite benchmark map: multiple environment cards, tool-use lanes, memory block, and scoring dashboard surround a central agent evaluator.. Show A reviewer comparing agent behaviors across blank tasks; no official logos, product UI, readable text, or red/green marks. only as supporting elements.
