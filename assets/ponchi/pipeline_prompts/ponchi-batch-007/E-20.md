...
scene_goal: make MMLU understandable as a simple book diagram
main_symbols: A broad knowledge benchmark map: many subject cards from different domains feed a model answer block, then a scored evaluation panel summarizes coverage.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for E-20 MMLU.

Subject: The entry subject is MMLU, but the visual subject is A broad knowledge benchmark map: many subject cards from different domains feed a model answer block, then a scored evaluation panel summarizes coverage.. Show A reviewer sorting generic domain cards; no official logos, school names, product UI, readable text, or red/green marks. only as supporting elements.
