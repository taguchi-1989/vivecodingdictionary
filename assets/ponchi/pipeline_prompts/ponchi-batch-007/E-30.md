...
scene_goal: make TAU-Bench understandable as a simple book diagram
main_symbols: An agent benchmark workflow: user goal, tool calls, simulated environment state, and task success scoring connect in an evaluation loop.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for E-30 TAU-Bench.

Subject: The entry subject is TAU-Bench, but the visual subject is An agent benchmark workflow: user goal, tool calls, simulated environment state, and task success scoring connect in an evaluation loop.. Show A reviewer watches an agent choose tools on generic cards; no official logos, product UI, readable text, or red/green marks. only as supporting elements.
