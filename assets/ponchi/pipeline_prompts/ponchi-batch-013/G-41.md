...
scene_goal: make Subagent understandable as a simple book diagram
main_symbols: A subagent delegation diagram: a parent agent workspace sends a task into a separate bounded context room, then receives a compact result back.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for G-41 Subagent.

Subject: The entry subject is Subagent, but the visual subject is A subagent delegation diagram: a parent agent workspace sends a task into a separate bounded context room, then receives a compact result back.. Show Two abstract assistant roles operate in separate boxes with a clear context boundary and return arrow; no product logos, no readable role names. only as supporting elements.
