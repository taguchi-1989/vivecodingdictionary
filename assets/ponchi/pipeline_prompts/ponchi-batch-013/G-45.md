...
scene_goal: make 段階的開示 understandable as a simple book diagram
main_symbols: A progressive disclosure interface diagram: a simple first layer shows only essential controls, while deeper optional panels unfold only when needed.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for G-45 段階的開示.

Subject: The entry subject is 段階的開示, but the visual subject is A progressive disclosure interface diagram: a simple first layer shows only essential controls, while deeper optional panels unfold only when needed.. Show A learner opens additional detail in stages instead of seeing every option at once; no product UI, no readable settings text, no brand marks. only as supporting elements.
