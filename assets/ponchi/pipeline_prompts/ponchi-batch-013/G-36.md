...
scene_goal: make Artifact understandable as a simple book diagram
main_symbols: A split workspace concept: conversation blocks on one side produce an editable output panel on the other, separated by a clear vertical workspace boundary.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for G-36 Artifact.

Subject: The entry subject is Artifact, but the visual subject is A split workspace concept: conversation blocks on one side produce an editable output panel on the other, separated by a clear vertical workspace boundary.. Show A non-engineer reviews and edits a generated code-or-document panel shown only as abstract blocks; no Claude.ai UI, no readable code, no brand marks. only as supporting elements.
