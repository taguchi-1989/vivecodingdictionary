...
scene_goal: make Worktree understandable as a simple book diagram
main_symbols: A worktree branching map: one central repository hub connects to three separate working directories where independent task lanes progress before merging back.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for G-42 Worktree.

Subject: The entry subject is Worktree, but the visual subject is A worktree branching map: one central repository hub connects to three separate working directories where independent task lanes progress before merging back.. Show Small abstract agent workers sit in separate folders to avoid interfering with each other; no mascots, no real repository UI, no readable branch names. only as supporting elements.
