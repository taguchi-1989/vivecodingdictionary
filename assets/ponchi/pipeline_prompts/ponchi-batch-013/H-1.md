...
scene_goal: make TDD understandable as a simple book diagram
main_symbols: A test-driven development loop: failing test card, minimal implementation card, and cleanup card cycle around a central code quality hub using abstract status symbols.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for H-1 TDD.

Subject: The entry subject is TDD, but the visual subject is A test-driven development loop: failing test card, minimal implementation card, and cleanup card cycle around a central code quality hub using abstract status symbols.. Show An engineer repeats the loop with changing expressions shown through simple posture only; use approved blue and grayscale status marks, no red or green colors, no readable test text. only as supporting elements.
