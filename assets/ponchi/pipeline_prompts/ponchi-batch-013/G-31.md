...
scene_goal: make Hook understandable as a simple book diagram
main_symbols: A hook automation flow: an assistant action passes through a pre-check gate, an execution block, and a post-check script gate before returning a status result.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for G-31 Hook.

Subject: The entry subject is Hook, but the visual subject is A hook automation flow: an assistant action passes through a pre-check gate, an execution block, and a post-check script gate before returning a status result.. Show A developer watches automatic checks run around a generic tool call; no Claude Code UI, no readable hook names, no terminal text. only as supporting elements.
