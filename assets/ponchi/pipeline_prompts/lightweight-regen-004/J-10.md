# J-10 Machine Learning ponchi prompt
...
scene_goal: make Machine Learning understandable as a simple book diagram
main_symbols: A machine-learning training loop: data examples flow into a model block, predictions are compared with outcomes, and feedback improves the next pass.
...
Subject: The entry subject is Machine Learning, but the visual subject is A machine-learning training loop: data examples flow into a model block, predictions are compared with outcomes, and feedback improves the next pass.. Show Show dataset cards, training arrows, evaluation gauges, and iteration loop; no code text, no framework logo, no readable numbers. only as supporting elements.
