...
scene_goal: make Serena MCP understandable as a simple book diagram
main_symbols: A symbol-aware code navigation comparison: before shows many files flooding context, after shows selected symbol-reference cards and impact graph nodes.
...
Subject: The entry subject is Serena MCP, but the visual subject is A symbol-aware code navigation comparison: before shows many files flooding context, after shows selected symbol-reference cards and impact graph nodes.. Show A developer compares broad file reading with focused symbol retrieval; no Serena logo, no readable code, no language logos. only as supporting elements.
