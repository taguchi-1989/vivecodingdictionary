...
scene_goal: make AWS MCP understandable as a simple book diagram
main_symbols: A cloud-resource connector fan-out: a client routes infrastructure request cards through a server group into generic storage, compute, database, and AI-service boxes.
...
Subject: The entry subject is AWS MCP, but the visual subject is A cloud-resource connector fan-out: a client routes infrastructure request cards through a server group into generic storage, compute, database, and AI-service boxes.. Show Show credential lock, resource fan-out, and confirmation cards; no AWS logo, no cloud provider icons, no console UI. only as supporting elements.
