...
scene_goal: make SQLite MCP understandable as a simple book diagram
main_symbols: A local database connector: natural-language request cards pass through a server gate into a local database file, returning table and chart result cards.
...
Subject: The entry subject is SQLite MCP, but the visual subject is A local database connector: natural-language request cards pass through a server gate into a local database file, returning table and chart result cards.. Show Show read-only versus write paths as abstract arrows and locks; no SQLite logo, no SQL text, no readable table data. only as supporting elements.
