# J-13 Transformer ponchi prompt
...
scene_goal: make Transformer understandable as a simple book diagram
main_symbols: A transformer attention block: token-like blank tiles interact through attention links, then pass through stacked processing blocks to produce an output sequence.
...
Subject: The entry subject is Transformer, but the visual subject is A transformer attention block: token-like blank tiles interact through attention links, then pass through stacked processing blocks to produce an output sequence.. Show Show parallel attention lines and residual-style flow as abstract arrows; no paper title, no formulas, no readable tokens. only as supporting elements.
