# I-10 Filesystem MCP ponchi prompt
...
scene_goal: make Filesystem MCP understandable as a simple book diagram
main_symbols: A filesystem access gate: a client request passes through a locked file-operation gate into an allowed folder, while a blocked folder remains inaccessible.
...
Subject: The entry subject is Filesystem MCP, but the visual subject is A filesystem access gate: a client request passes through a locked file-operation gate into an allowed folder, while a blocked folder remains inaccessible.. Show Show read, write, and list operations only as abstract arrows and blank file cards; no Claude logo, no terminal text, no path names. only as supporting elements.
