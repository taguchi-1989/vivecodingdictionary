...
scene_goal: make Sycophancy understandable as a simple book diagram
main_symbols: A sycophancy behavior comparison: one path blindly agrees with a user card, while a better path checks evidence and politely corrects the answer.
...
Subject: The entry subject is Sycophancy, but the visual subject is A sycophancy behavior comparison: one path blindly agrees with a user card, while a better path checks evidence and politely corrects the answer.. Show Use agreement bubbles, evidence cards, and correction loop as abstract panels; no readable dialog, no product UI. only as supporting elements.
