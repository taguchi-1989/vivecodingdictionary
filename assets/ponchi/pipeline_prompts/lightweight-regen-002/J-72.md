...
scene_goal: make H100 understandable as a simple book diagram
main_symbols: A high-end accelerator workload map: large model training cards flow through many parallel compute lanes on a generic accelerator board.
...
Subject: The entry subject is H100, but the visual subject is A high-end accelerator workload map: large model training cards flow through many parallel compute lanes on a generic accelerator board.. Show Use abstract chip modules and cooling-flow lines; no NVIDIA logo, no product badge, no readable model name. only as supporting elements.
