# I-22 Chrome DevTools MCP ponchi prompt
...
scene_goal: make Chrome DevTools MCP understandable as a simple book diagram
main_symbols: A browser-performance measurement flow: an agent request passes through a devtools connector into a generic browser, then returns console, trace, and performance chart cards.
...
Subject: The entry subject is Chrome DevTools MCP, but the visual subject is A browser-performance measurement flow: an agent request passes through a devtools connector into a generic browser, then returns console, trace, and performance chart cards.. Show Show metric gauges and trace blocks only as abstract graphics; no Chrome logo, no DevTools UI, no readable metric text. only as supporting elements.
