# I-24 Context7 MCP ponchi prompt
...
scene_goal: make Context7 MCP understandable as a simple book diagram
main_symbols: A current-docs retrieval comparison: an outdated-memory answer path contrasts with a path that retrieves fresh official documentation cards before answering.
...
Subject: The entry subject is Context7 MCP, but the visual subject is A current-docs retrieval comparison: an outdated-memory answer path contrasts with a path that retrieves fresh official documentation cards before answering.. Show Show blank prompt, docs, and answer cards with a freshness gauge; no Context7 logo, no library logos, no readable code. only as supporting elements.

Composition: Use timeline_scale / diagram_first with balanced density. The main subject must occupy more than half of the canvas width. Use 2-4 large visual blocks rather than many tiny cards.
