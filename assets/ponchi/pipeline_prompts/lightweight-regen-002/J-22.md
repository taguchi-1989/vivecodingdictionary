...
scene_goal: make パラメータ数の単位 understandable as a simple book diagram
main_symbols: A parameter-scale comparison: nested model blocks grow from small to huge, with memory and compute requirement cards increasing alongside them.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for J-22 パラメータ数の単位.

Subject: The entry subject is パラメータ数の単位, but the visual subject is A parameter-scale comparison: nested model blocks grow from small to huge, with memory and compute requirement cards increasing alongside them.. Show Use abstract size stacks and gauge bars; no readable units, no numbers, no brand or hardware logos. only as supporting elements.
