...
scene_goal: make ノイマン型 understandable as a simple book diagram
main_symbols: A von Neumann architecture diagram: memory, control, arithmetic, input, and output blocks exchange signals through a central bus.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for J-32 ノイマン型.

Subject: The entry subject is ノイマン型, but the visual subject is A von Neumann architecture diagram: memory, control, arithmetic, input, and output blocks exchange signals through a central bus.. Show Use clean abstract hardware blocks and data arrows; no readable labels, no chip vendor logos, no equations. only as supporting elements.
