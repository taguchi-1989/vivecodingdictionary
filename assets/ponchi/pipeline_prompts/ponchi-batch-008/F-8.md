...
scene_goal: make JSON understandable as a simple book diagram
main_symbols: A data exchange workflow: API response blocks become nested object cards, list arrays, and application state panels.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for F-8 JSON.

Subject: The entry subject is JSON, but the visual subject is A data exchange workflow: API response blocks become nested object cards, list arrays, and application state panels.. Show A developer traces structured data between server and client blocks; no logos, product UI, or readable keys. only as supporting elements.
