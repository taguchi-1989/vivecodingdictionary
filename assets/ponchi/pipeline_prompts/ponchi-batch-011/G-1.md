# G-1 Context (コンテキスト) ponchi prompt
...
scene_goal: make Context (コンテキスト) understandable as a simple book diagram
main_symbols: A context window concept map: user intent card, conversation history blocks, retrieved notes, and current model response panel connect into one working memory area.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for G-1 Context (コンテキスト).

Subject: The entry subject is Context (コンテキスト), but the visual subject is A context window concept map: user intent card, conversation history blocks, retrieved notes, and current model response panel connect into one working memory area.. Show A person shows how relevant background changes an assistant's answer; no AI-company logos, chat product UI, readable text, or brand icons. only as supporting elements.
