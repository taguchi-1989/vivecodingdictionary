...
scene_goal: make OAuth understandable as a simple book diagram
main_symbols: An authorization handoff: user device, app server, consent gate, token card, and resource server connect through secure arrows.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for F-130 OAuth.

Subject: The entry subject is OAuth, but the visual subject is An authorization handoff: user device, app server, consent gate, token card, and resource server connect through secure arrows.. Show A developer checks how access is delegated without sharing a password; no provider logos, real login UI, readable tokens, or brand colors. only as supporting elements.
