...
scene_goal: make SSR understandable as a simple book diagram
main_symbols: A server-side rendering flow: request card goes to server render block, HTML-like page shell, hydration step, and browser display panel.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for F-161 SSR.

Subject: The entry subject is SSR, but the visual subject is A server-side rendering flow: request card goes to server render block, HTML-like page shell, hydration step, and browser display panel.. Show A developer compares response speed and client activation; no framework logos, browser logos, readable code, or real product UI. only as supporting elements.
