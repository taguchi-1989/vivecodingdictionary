...
scene_goal: make SSG understandable as a simple book diagram
main_symbols: A static site generation flow: content files and template blocks feed a build machine, producing prebuilt page cards served from a generic hosting box.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for F-162 SSG.

Subject: The entry subject is SSG, but the visual subject is A static site generation flow: content files and template blocks feed a build machine, producing prebuilt page cards served from a generic hosting box.. Show A developer prepares pages before deployment; no framework logos, host logos, readable code, or real product UI. only as supporting elements.
