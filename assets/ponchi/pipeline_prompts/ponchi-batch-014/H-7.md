...
scene_goal: make CI/CD understandable as a simple book diagram
main_symbols: A deployment pipeline: code cards move through test, build, and deploy gates, then reach a user-facing result panel with a monitoring check loop.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for H-7 CI/CD.

Subject: The entry subject is CI/CD, but the visual subject is A deployment pipeline: code cards move through test, build, and deploy gates, then reach a user-facing result panel with a monitoring check loop.. Show A developer starts the pipeline and a user receives the updated generic app card; no service logos, no terminal text, no product UI. only as supporting elements.
