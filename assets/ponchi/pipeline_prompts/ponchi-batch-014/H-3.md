# H-3 バイブコーディングの流儀 ponchi prompt
...
scene_goal: make バイブコーディングの流儀 understandable as a simple book diagram
main_symbols: A vibe-coding practice loop: goal card, assistant generation block, human verification, small correction loop, and cleanup checkpoint connect as a repeatable workflow.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for H-3 バイブコーディングの流儀.

Subject: The entry subject is バイブコーディングの流儀, but the visual subject is A vibe-coding practice loop: goal card, assistant generation block, human verification, small correction loop, and cleanup checkpoint connect as a repeatable workflow.. Show A non-engineer steers the loop with intent cards and verification tools; no provider logos, no chat UI, no readable code. only as supporting elements.
