...
scene_goal: make Git Flow understandable as a simple book diagram
main_symbols: A branching strategy comparison: a complex multi-branch lane on one side contrasts with a simpler two-lane flow on the other, with a developer choosing the simpler path.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for H-6 Git Flow.

Subject: The entry subject is Git Flow, but the visual subject is A branching strategy comparison: a complex multi-branch lane on one side contrasts with a simpler two-lane flow on the other, with a developer choosing the simpler path.. Show Use abstract branch lines, merge nodes, release tags, and cleanup symbols; no GitHub logo, no branch names, no readable labels. only as supporting elements.
