...
scene_goal: make ペアプログラミング understandable as a simple book diagram
main_symbols: A paired coding workspace: two people share one large neutral code workspace, one points at blank code blocks while the other types, with feedback arrows between them.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for H-2 ペアプログラミング.

Subject: The entry subject is ペアプログラミング, but the visual subject is A paired coding workspace: two people share one large neutral code workspace, one points at blank code blocks while the other types, with feedback arrows between them.. Show Show collaboration, shared attention, and quick correction using blank cards and check marks; no readable code, no product UI, no brand marks. only as supporting elements.
