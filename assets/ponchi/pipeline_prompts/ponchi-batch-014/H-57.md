# H-57 Gemini の命名史 ponchi prompt
...
scene_goal: make Gemini の命名史 understandable as a simple book diagram
main_symbols: A naming-history timeline: an earlier experimental name tile transitions through a rename marker into a later unified model-family tile with clearer identity.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for H-57 Gemini の命名史.

Subject: The entry subject is Gemini の命名史, but the visual subject is A naming-history timeline: an earlier experimental name tile transitions through a rename marker into a later unified model-family tile with clearer identity.. Show A reader traces the naming change using abstract cards and arrows; no Google colors, no Gemini logo, no Bard logo, no readable names. only as supporting elements.
