...
scene_goal: make コードレビュー understandable as a simple book diagram
main_symbols: A code review workflow: a change bundle moves from author workspace to reviewer checklist, then returns with feedback marks before entering an accepted result box.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for H-4 コードレビュー.

Subject: The entry subject is コードレビュー, but the visual subject is A code review workflow: a change bundle moves from author workspace to reviewer checklist, then returns with feedback marks before entering an accepted result box.. Show One author and one reviewer compare blank diffs, risk markers, and check cards; no readable code, no repository UI, no brand marks. only as supporting elements.
