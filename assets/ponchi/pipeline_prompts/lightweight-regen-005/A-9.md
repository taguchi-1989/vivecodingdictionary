...
scene_goal: make 索引 understandable as a simple book diagram
main_symbols: An index navigation map: many entry cards connect through alphabetical-style tabs, cross-links, and search paths into a chosen destination card.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for A-9 索引.

Subject: The entry subject is 索引, but the visual subject is An index navigation map: many entry cards connect through alphabetical-style tabs, cross-links, and search paths into a chosen destination card.. Show Use tab shapes and reference arrows with no letters or readable words; no product search UI. only as supporting elements.
