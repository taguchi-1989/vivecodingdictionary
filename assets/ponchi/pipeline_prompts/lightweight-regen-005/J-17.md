...
scene_goal: make Attention understandable as a simple book diagram
main_symbols: An attention mechanism map: one query tile scans many context tiles, highlighting the most relevant connections before producing a focused output.
...
Subject: The entry subject is Attention, but the visual subject is An attention mechanism map: one query tile scans many context tiles, highlighting the most relevant connections before producing a focused output.. Show Use spotlight beams, weighted arcs, and selected tiles; no formulas, no readable text, no paper marks. only as supporting elements.
