...
scene_goal: make LLM understandable as a simple book diagram
main_symbols: A language-model context window: many blank word tiles enter a large model core, which predicts and composes the next response card.
...
Subject: The entry subject is LLM, but the visual subject is A language-model context window: many blank word tiles enter a large model core, which predicts and composes the next response card.. Show Show context, generation, and feedback cards as abstract tiles; no chat UI, no product logos, no readable words. only as supporting elements.
