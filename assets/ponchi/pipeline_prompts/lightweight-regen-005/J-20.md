...
scene_goal: make Big Data understandable as a simple book diagram
main_symbols: A big-data pipeline: many data streams gather into a lake-like storage hub, then feed analysis, model training, and decision cards.
...
Subject: The entry subject is Big Data, but the visual subject is A big-data pipeline: many data streams gather into a lake-like storage hub, then feed analysis, model training, and decision cards.. Show Show scale through many source tiles, filtering gates, and aggregate dashboards with blank charts; no database logos, no readable metrics. only as supporting elements.
