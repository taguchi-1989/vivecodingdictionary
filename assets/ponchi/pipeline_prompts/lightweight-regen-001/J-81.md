...
scene_goal: make M.2 understandable as a simple book diagram
main_symbols: A compact storage-module diagram: a slim card plugs directly into a board slot, using short high-speed lanes to reach the processor and memory.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for J-81 M.2.

Subject: The entry subject is M.2, but the visual subject is A compact storage-module diagram: a slim card plugs directly into a board slot, using short high-speed lanes to reach the processor and memory.. Show Show generic module, keyed slot, and lane arrows; no standard logo, no readable key labels, no manufacturer mark. only as supporting elements.
