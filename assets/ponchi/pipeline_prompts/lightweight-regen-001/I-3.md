...
scene_goal: make MCP Client understandable as a simple book diagram
main_symbols: A client-side connection window: a host application gathers available server tools, selects one route, and receives result cards back.
...
Subject: The entry subject is MCP Client, but the visual subject is A client-side connection window: a host application gathers available server tools, selects one route, and receives result cards back.. Show Show the client as a neutral control panel that calls multiple generic servers; no Claude, Cursor, or editor logos; no readable settings text. only as supporting elements.
