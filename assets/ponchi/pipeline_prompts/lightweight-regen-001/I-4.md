# I-4 MCP Transport ponchi prompt
...
scene_goal: make MCP Transport understandable as a simple book diagram
main_symbols: A transport comparison: one local process lane and one remote network lane carry the same protocol messages between client and server.
...
Subject: The entry subject is MCP Transport, but the visual subject is A transport comparison: one local process lane and one remote network lane carry the same protocol messages between client and server.. Show Use two parallel lanes with local computer/process icons versus cloud-like endpoint boxes; no command text, no URLs, no protocol labels. only as supporting elements.
