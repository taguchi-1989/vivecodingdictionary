# G-13 Few-shot Learning ponchi prompt
...
subject_stack:
  entry_subject: Few-shot Learning
...
scene_goal: make Few-shot Learning understandable as a simple book diagram
main_symbols: A few-shot learning setup: several example input-output cards establish a pattern, then a new blank input follows the same output path.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for G-13 Few-shot Learning.

Subject: The entry subject is Few-shot Learning, but the visual subject is A few-shot learning setup: several example input-output cards establish a pattern, then a new blank input follows the same output path.. Show A learner provides multiple demonstrations before the assistant generalizes; no model/provider logos, no readable words, no chat UI. only as supporting elements.
