...
scene_goal: make RAG understandable as a simple book diagram
main_symbols: A retrieval-augmented generation workflow: user question goes to search index, retrieves document cards, then feeds an assistant answer panel.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for G-15 RAG.

Subject: The entry subject is RAG, but the visual subject is A retrieval-augmented generation workflow: user question goes to search index, retrieves document cards, then feeds an assistant answer panel.. Show A developer connects relevant knowledge to generation; no model/provider logos, no search-engine UI, no readable text. only as supporting elements.
