...
subject_stack:
  ...
  logo_subject: official SQLite brand overlay after source review
...
clearspace:
  required: yes
  location: rightmost 40% of top 25% kept completely blank for official SQLite logo
...
scene_goal: make SQLite understandable as a simple book diagram
main_symbols: An embedded database workflow: local app file connects to compact database card, query block, cache panel, and device storage.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for F-121 SQLite.

Subject: The entry subject is SQLite, but the visual subject is An embedded database workflow: local app file connects to compact database card, query block, cache panel, and device storage.. Show A developer uses a lightweight local database; no SQLite logo, product UI, readable table names, or SQL text. only as supporting elements.
