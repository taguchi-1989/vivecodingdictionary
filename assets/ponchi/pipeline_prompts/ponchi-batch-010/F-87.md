...
scene_goal: make sudo understandable as a simple book diagram
main_symbols: A permission escalation workflow: normal user task card passes through a privilege gate, approval shield, and protected system action panel.
...
Primary request: Create a 2:1 horizontal technical-book ponchi illustration for F-87 sudo.

Subject: The entry subject is sudo, but the visual subject is A permission escalation workflow: normal user task card passes through a privilege gate, approval shield, and protected system action panel.. Show A developer carefully performs an admin action; no logos, product UI, readable commands, or red warning color. only as supporting elements.
