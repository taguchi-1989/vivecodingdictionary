...
1. **tagline**（許容 **25〜60 字**、推奨 30〜38、略称展開含む場合 35〜50、1〜2 文）：用語を言い切る。結論ファースト、です・ます調、カタカナは読み仮名。**旧「ひとことで」は本セクションに合流**。
   - **タイトルが略称・ヌメロニム（MCP / a11y / LLM 等）なら、冒頭に正式名称または日本語展開を 1 句入れる**。形は「{展開} の略。{定義 1 文}。」が基準。例：`Model Context Protocol の略。LLM とツール・データをつなぐ標準規格です。` / `accessibility の略（a と y の間に 11 文字）。Web を誰でも使える状態に保つ取り組みです。`
   - `title_reading` は「英単語をカタカナで並べた読み」（例：`モデルコンテキストプロトコル` / `ラージランゲージモデル` / `アクセシビリティ`）。略称音読み・日本語訳は使わず、正式名称は tagline へ回す
...
5. **会話での使い方例**（**30〜40 字、1 文**）：「わかってる人風」が自然にこの語を会話に織り込んだ一例。関連語を 1〜3 個まで組み合わせてよい。嫌味・煽りは避け、です・ます調。下チロム右スロットに印字（v2_rules_summary §1）
6. **関連用語**（3〜5 個、合計 20〜50 字）：誌面ではタグチップ表示。短い単語中心
...
## 書きすぎ防止チェック（2026-04-29 追加）

validator が走る前に、以下の 4 点を執筆中・保存前に頭で確認します。これを通すと修正サイクルがほぼ消えます。
...
誌面ではタグ／ピル表示なので説明テキストは出ません。説明したい場合は **「## コミュニティ補完メモ」** に逃がします（例：「Cursor（B-4）との住み分け：…」）。許容字数 20〜50 を 100 字以上に膨らませる原因の 9 割がこのパターンです。

### 2. 左右ページ合計を予算で押さえる（書く前に決める）

書き始める前に、各セクションの目標字数を **中央値で見積もる** こと。許容上限ギリギリ狙いはしません。
...
**よくあるミス**：許容上限の 200 字を 2 つ書くと左ページ合計が 460 字になり、☆ 違反（合計上限 +100 字超）に直結します。

### 3. 書きたい補足は裏台帳に逃がす

誌面に出ない 4 つの逃がし先：

- **コミュニティ補完メモ**：近接エントリとのスコープ分担、補足説明
- **誌面ポンチ絵メモ**：図の指示
- **出典メモ**：URL ＋ checked 日付
- **備考**：時期差分・例外・注記

「もう少し書きたい」と思った時点で本文の字数予算を超えやすいので、まずここに置く判断を先にしてください。

### 4. 保存前のセルフチェック（30 秒）

- [ ] `## 関連用語` の各行が `- 用語名` だけ（em-dash・カッコ書きの説明なし）
- [ ] tagline + 何をしてくれるか + どこで出会うか の合計が 250 字以内に収まる見込み
- [ ] 6 視点 6 つは各 30 字以内（許容 40 字）
- [ ] 著者欄（非エンジニアのつまずき／私のコメント）は空のまま
- [ ] 「会話での使い方例」が `です・ます` 語尾で終わる（引用「」内であっても）
- [ ] tagline が通常タイトルなら ≤ 38 字推奨（略称展開時のみ 50〜60 字許容）

ここを通せば validator が `drafting → needs_review` に一発昇格させます。

### 5. 並列実行で頻発する警告パターン（2026-04-29 学習）

`.claude/agents/entry-writer.md` §「書きすぎ防止チェック」§5「並列実行で頻発する警告パターン」を参照。
特に下記 3 点が並列起動時に同時発生しやすいので、書き始める前に確認:

1. **「会話での使い方例」の文末**: 引用「」の中身も `です・ます` 語尾。validator は `」` 直前の語尾を評価する
2. **6 視点 ≤ 40 字**: 特に `3. 注意点` `4. どこで役立つか` `5. はじめに` で超えやすい。列挙はコミュニティ補完メモへ逃がす
3. **tagline ≤ 45 字**: 通常タイトルは 38 字を目標。略称展開時のみ 50〜60 字許容
...
- **タイトルが略称・ヌメロニムなら tagline 冒頭に正式名称**（`Model Context Protocol の略。…`）。`title_reading` は「英単語をカタカナで並べた読み」（`モデルコンテキストプロトコル` 等）
...
- v0.4（2026-04-29）: 「書きすぎ防止チェック」セクション追加。validator の手戻りで最頻出だった 2 パターン（関連用語の em-dash 説明、左右ページ合計の上限ギリギリ狙い）を予防する形でガードレール化。
