...
順番厳守。**左ページ本文の合計は 700〜800 字が目安**（メイン図が誌面の 40% を使うため）。超えたら Step 7 のチェックで削る。

1. **tagline**（**25〜40 字**）：青帯に乗る。結論ファースト、です・ます調、カタカナは読み仮名
2. **ひとことで**（**50〜80 字**、1〜2 文）：会話の入り口として単独で意味が通る定義
3. **何をしてくれるか**（**250〜350 字**、必要なら構造化）：役割と仕組みを具体的に。長いコードは貼らない
4. **バイブコーディングでの位置づけ**（**180〜280 字**）：AI との打ち合わせ・指示・現場での立ち位置。業務意味そのままにしない。**著者の個人的エピソードは「私のコメント」に回す**
5. **メイン図**：`figure_type` に応じた A / B / C のどれか 1 つだけ残す。「図の狙い」を 1〜2 文で添える
6. **関連用語**（3〜6 個、合計 100〜150 字）：`用語名 — 短いメモ` 形式。YAML と一致

### Step 4. 右ページを書く

**右ページ本文の合計は 250〜400 字が目安**。

1. **この用語の見どころ（6 視点）**：1.役割 / 2.うれしさ / 3.注意点 / 4.どこで役立つか / 5.最初に理解する範囲 / 6.深掘り先。**各 1 文・20〜40 字目安**（合計 150〜240 字）
...
- `非エンジニア視点のつまずき`：`-\n-\n-` のまま
- `私のコメント`：`🙂 第一印象:` `👍 良い点:` `👎 ダメな点:` `🎯 誰に向くか:` のラベルだけ残す
...
- **字数目安を超えない**：左ページ本文 700〜800 字、右ページ本文 250〜400 字。超えたら重複・著者エピソード・他エントリ領域を優先して削る（checklist §H 参照）
