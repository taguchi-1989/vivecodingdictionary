...
*2026-04-23 起草、2026-04-28 改訂。スケルトン先行運用に切り替えました（旧ステージ 1〜3 構造は参考として下に残します）。*

## 現行運用：スケルトン先行（2026-04-28〜）

全 339 件のうち約 318 件をまず **スケルトン状態**（status: skeleton）で生成済み。本書きは以下の手順で 1 件ずつ昇格させます。

1. 着手するエントリを選ぶ
2. md ファイルを開き、YAML の `status: skeleton` を `status: drafting` に変更
3. `entry-writer` サブエージェント（[.claude/agents/entry-writer.md](../.claude/agents/entry-writer.md)）を呼ぶ。
   呼び方：「B-4 Cursor を書いて」のようにエントリ ID を渡す
4. validator が走り、字数・構造・トーンを機械チェック
5. レビュー OK なら `status: needs_review` → `ready` と昇格

新しい候補を追加したいときは、`ledgers/entries.csv` に行を追加して `python3 scripts/generate_skeleton.py {ID}` で 1 件だけスケルトン化できます。既存ファイルは絶対に上書きされません。
...
下の旧ステージ表は、以前「先に書く 15＋46 件」を選んだときの記録として残しています。スケルトンが全部そろった今は、letter スプリント（letter ごと完走）か、ステージ 2 の 46 件をリストとしてそのまま使うのが自然です。
