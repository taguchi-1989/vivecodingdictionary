...
## 3. 紙面構成の確定（iter 7-8 で確定）

### 左ページ（p.04 相当）

1. チロム（book icon + 技術用語ラベル / ページ番号）
2. **タイトル** `{{title}}` — `title-hero` 96px ネイビー
3. **タグライン帯** — 25〜40 字、淡青塗り、title の言い換えではなく一歩先の定義
4. **タグ行** — 体験区分 ／ 推奨読者レベル の 2 ピル
5. **本文セクション 2 つ**（ひとことで は削除）
   - 何をしてくれるか — 役割と仕組み、2〜4 文
   - どこで出会うか — 現場のどこで名前を聞くか、2〜4 文
6. **Before/After** — 親＋子の二層カード（または図の代替）
7. **関連用語** — 3〜5 ピル
8. （下部空白は将来の「イメージ／写真」スロット用に保留）

### 右ページ（p.05 相当）

1. チロム（book icon + `{{title}} の見方` / ページ番号）
2. **タイトル** `{{title}} をどう読むか` — `title-hero--right` 50px ネイビー
3. （サブタイトル削除、前付け「この図鑑の読み方」に集約）
4. **セクション見出し**「この用語の見どころ」
5. **6 視点グリッド** — 役割 ／ うれしさ ／ 注意点 ／ どこで効くか ／ **はじめに** ／ 深掘り先（「最初に理解する範囲」は長いので「はじめに」に短縮、iter 6）
6. **2 カラム下段**
   - 非エンジニア視点のつまずき — 青ドット箇条書き
   - 私のコメント — ★/✓/!/☺ バッジ＋ラベル＋本文
7. **開発フローでの位置** — 4 ステップ横並び、アイコン＋ラベル＋矢印
8. **下チロム** — book icon + 「バイブコーディング図鑑」横並び
...
| （新規） ポンチ絵スロット | なし | 左ページ Before/After と関連用語の間、青 dashed 枠の placeholder として追加 |
