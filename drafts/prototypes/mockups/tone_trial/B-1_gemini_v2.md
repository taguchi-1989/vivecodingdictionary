...
<!-- 左ページ冒頭の可視タグ行（Design System v2 サンプル p.04 の「体験区分／推奨読者レベル」バッジに対応）。 -->
...
Gemini は、Google が作る AI アシスタントのブランドです。ひとつのアプリではなく、ウェブ・スマホ・業務・開発と複数の入口に広がっています。
...
<!-- 旧「バイブコーディングでの位置づけ」。Design System v2 サンプル p.04 の節名に揃えます。 -->

Claude と並ぶ主役級のサービスで、使い手ごとに別の顔を見せるのが特徴です。企画なら Workspace の Docs／Gmail、開発なら Vertex AI、個人利用なら gemini.google.com や Android の標準 AI、というふうに入口が分かれます。
...
<!-- 右ページ冒頭。Design System v2 サンプル p.05 の「{{title}} をどう読むか」＋1行サブタイトル。 -->

定義だけでなく、どの入口で誰が使うのか・どのモデルを選ぶのかをセットで見ると掴みやすくなります。

## この用語の見どころ

### 1. 役割

Google 製の汎用 AI アシスタントで、個人・業務・開発の全方位に入口があります。

### 2. うれしさ

Google Workspace と深く統合され、Docs ／ Gmail から自然に呼び出せます。

### 3. 注意点

サービス名・モデル名・API 窓口が同じ「Gemini」の下に並び、会話がずれやすくなります。

### 4. どこで効くか

<!-- 旧「どこで役立つか」。Design System v2 サンプル p.05 の節名に揃えます。 -->

会議メモ要約、長い PDF の読み込み、Android の音声操作、Cloud 上の業務組み込みで効きます。

### 5. 最初に理解する範囲

4 つの入口と、Pro ／ Flash ／ Flash-Lite の 3 段階モデルまで押さえれば入口が揃います。

### 6. 深掘り先

次に読むなら Gemini 2.5 系、Vertex AI、AI Studio、Google Workspace 個別機能、Context Window です。

## 開発フローでの位置（必須）

1. 目的を決める — 調べ物／業務文書／アプリ実装のどれか
2. 入口を選ぶ — チャット／Android／Workspace／API
3. モデルを選ぶ — Pro（重い仕事）／Flash（日常）／Flash-Lite（軽量大量）
4. 実行して評価し、必要なら入口やモデルを切り替え
...
- **この版は tone_trial 用のコピーで、status: trial を付与**。原本 `content/entries/service/B-1_gemini.md` は未変更
