...
- Hobby は個人限定で商用は Pro（月 20 ドル〜）必須。ビルド時間が嵩むと CI/CD だけで毎月溶けます
- API キーなどの環境変数をダッシュボードで設定する作法に慣れるまで、ローカル動作との差で戸惑います

<!-- AUTHOR: user_only / AI-ASSIST: no -->
## 私のコメント

- 🙂 第一印象: Web アプリを作らせると Next.js のデプロイ先として真っ先に勧められます
- 👍 良い点: 環境変数を入れれば Supabase や Discord など外部サービス連携も完結します
