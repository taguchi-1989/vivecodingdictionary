...
- 導線が分かりにくく、AWS 自体の知識（IAM・コンソール操作）がないと最初の API 呼び出しまで辿り着けません
- OpenAI のモデルは長らく Bedrock では使えませんでしたが、2026 年に gpt-oss や GPT 系が追加され、状況が変わりつつあります
- 業務情報を入れて本当にセキュアに本番運用できるのか、ガバナンス面の判断材料が分かりづらいです

<!-- AUTHOR: user_only / AI-ASSIST: no -->
## 私のコメント

- 🙂 第一印象: LLM に「API をセキュアに業務利用したい」と相談すると候補に挙がってきました
- 👍 良い点: エンタープライズレベルのセキュリティ・データ保護要件を満たした構成にできるそうです
- 👎 ダメな点: 個人ではまだ実装できておらず、LiteLLM のような複数 LLM 統合ツールとの差別化点を掴みきれていません
- 👥 誰向けか: 企業の情報システム・インフラを担当するエンジニア向けです
