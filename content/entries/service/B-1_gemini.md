...
自然言語で話しかけると、要約・調査・コード生成・画像理解などを返してくれます。ブラウザのチャット、Workspace 統合、Vertex AI 経由の API と、複数の入口を同じブランドで持つのが特徴です。

## どこで出会うか

Claude と並ぶ主役級のサービスで、企画・開発・業務・個人利用と複数の顔があります。「Gemini を使っている」と言ったとき、どの入口の話かを揃えると会話が噛み合いやすくなります。
...
同じ「Gemini」の下にサービス・モデル・API 窓口が並び会話がずれやすい。

### 4. どこで役立つか

会議メモ要約、長 PDF の読み込み、Cloud 上の業務組み込み。

### 5. はじめに

4 つの入口と、Pro／Flash／Flash-Lite の 3 段階。
...
- プレビュー期間が長くモデルが安定しないので、業務で使うときの運用負荷が地味に重いです
- Gemini まわりの名前（プラン名／Workspace 連携の呼び方）が頻繁に変わるので、過去の情報がすぐ古くなります
- Google ＝ 検索が強い、というイメージから期待してしまいますが、ハルシネーションは他社モデルと比べてもむしろ多めで、ギャップに戸惑います
