...
プロジェクトのルートや `~/.claude/` に置くと、CC がセッション開始時に読み込みます。コーディング規約・注意事項を一度書いておくと、毎回 System Prompt（システムプロンプト）に手入力しなくても CC が文脈を把握した状態で動き始めます。

## どこで出会うか

CC を初めてプロジェクトで使うときに紹介される場面が多いです。本書プロジェクト自身もルートに置いており、規約が毎セッション CC に伝わります。チームで使う場合は Git に入れて共有します。
...
1. プロジェクト規約を整理 — 言語・スタイル・禁止事項を箇条書きにまとめる
2. CLAUDE.md を作成 — プロジェクトルートに置き、規約と注意点を記述する
3. CC がセッション開始時に読む — 追加指示なしで CC がプロジェクトの文脈を把握する
4. 規約変更時に更新 — チームの合意を得てファイルを修正し Git でレビューする
5. 肥大化を防ぐ — 不要になったルールを削り、Context の圧迫を定期的に解消する
