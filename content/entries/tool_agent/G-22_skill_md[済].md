...
- スキルを使う場合と Claude Code の動作設定で対応する場合の切り分けが難しいです。モデルに委任して短い指示にまとめるなど、設計が必要になります。
- 
- 
...
- 🙂 第一印象: なぜこれを使わないといけないのかが最初はあまり理解できませんでした。
- 👍 良い点: 必要なタイミングで必要なものを提供できるようにできることです。
- 👎 ダメな点: メンテナンスしないと複雑になりますし、作りっぱなしにもできないのが大変です。
- 👥 誰向けか: バイブコーダーでエージェントをある程度自律化させた人なら全員必要だと思います。
