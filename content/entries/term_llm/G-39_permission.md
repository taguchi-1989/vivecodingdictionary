...
Claude Code（CC）がツール（Bash / Edit / Write / MCP）を呼ぶたびに、設定ファイルのルールを参照して実行を許可・拒否・確認のいずれかに振り分けます。事前に `allow` リストを整えると、毎回の確認が省けます。

## どこで出会うか

`.claude/settings.json`（プロジェクト共有）や `~/.claude/settings.json`（ユーザー全体）を開いたときに `permissions` キーとして現れます。チームで CC を使い始めると「この操作は毎回確認が入る」と気づき、ルールを整えるきっかけになります。
...
「Permission に `git status` だけ allow しておくと、毎回確認されずスムーズです。」
...
1. 設定ファイル作成 — `.claude/settings.json` に `permissions` セクションを追加します
2. ルール記述 — `allow` に毎回 OK な操作、`deny` に禁止操作を列挙します
3. CC 起動 — ツール呼び出し時にルールが自動参照されます
4. 運用調整 — `ask` が増えた操作を `allow` に昇格してリストを育てます
5. チーム共有 — settings.json を Git に commit してチームで統一します
