...
- 昔は Stable Diffusion などの画像生成でネガティブプロンプトをガチガチに書くのが流行り、「段階的に考えて」みたいな呪文も多かった。
- 今はモデル側に学習が入っていて、そういう細かい指示が要らないことも多い。「いい感じで」と曖昧に渡した方が良い結果になる場面も増えた。
- どこまで指示を絞って、どこまで AI に任せるか、その切り分けが難しい。

<!-- AUTHOR: user_only / AI-ASSIST: no -->
## 私のコメント

- 🙂 第一印象: 絵を描くために必要な呪文、というイメージ。
- 👍 良い点: 仕事ではまだハーネス化されていない場面が多く、しっかりやりたいことを言う必要はある。
- 👎 ダメな点: 最適な指示の形がモデルの賢さで変わるので、いつまでベストかが分からず難しい。
- 👥 誰向けか: 今はそこまで流行りという印象ではない。ハーネス／Context Engineering に内包されつつある感じ。
