...
Retrieval-Augmented Generation（検索拡張生成）の略。LLM が回答する直前に外部文書を検索して文脈に注入する手法です。

<!-- ━━━━━━━━ 左ページ ━━━━━━━━ -->

## 何をしてくれるか

ユーザーの質問を Embedding（G-16）でベクトル化し、ベクトル DB（G-17）から類似文書を取得して LLM のプロンプトに添えます。これにより社内規定や専門書など、学習データに含まれない知識を根拠にした回答が得られます。

## どこで出会うか

社内 FAQ ボットや製品マニュアルの Q&A システムを構築する場面で登場します。「Fine-tuning（J-16）より手早く試せて、根拠文書も明示できる」として、RAG の名前をドキュメントやスライドで見かけることがあります。
