...
- どこまで委任するかが鍵。Claude Code でも、昔は「Yes をひたすら押す」状態の管理で手いっぱいになっていた。
- 一方で `--dangerously-skip-permissions` のような全許可で動かしてしまう人も自分を含めいるはずで、Permission 設定は結構鍵だった。
- 今は一定の規律を守りながら動いてくれる仕組みも出てきていて、技術的にだいぶ解決されてきた印象。

<!-- AUTHOR: user_only / AI-ASSIST: no -->
## 私のコメント

- 🙂 第一印象: バイブコーディングなので、とりあえず全部委任したいと思った。
- 👍 良い点: しっかり設定すれば、本来やってはいけない危険な操作（リポジトリを吹っ飛ばすような機能）は不許可にできる。
- 👎 ダメな点: 何も考えずに skip permissions を選んで吹っ飛ばされる事故が昔はあった。今はほぼないと思う。
