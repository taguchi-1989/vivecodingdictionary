...
Tool Use（ツールユース）とは、LLM（大規模言語モデル）が「このツールを使いたい」という意図を返し、ホスト側がツールを実行して結果をモデルに戻す仕組みです。Web 検索やコード実行など、LLM 単体では完結しない処理を組み込めます。

## どこで出会うか

Anthropic の Claude API や OpenAI の API のドキュメントで「Tool Use」「Function Calling」として紹介されています。Claude Code や ChatGPT Plugins など、AI が外部ツールを動かすサービスを使うときに、この仕組みが裏で動いています。
