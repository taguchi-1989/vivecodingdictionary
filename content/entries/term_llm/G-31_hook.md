...
Claude Code がツールを呼ぶ直前・直後に、指定したシェルスクリプトを自動実行する仕組みです。「保存のたびに validator を走らせる」「応答後に Slack へ通知する」といった処理を AI の返答と切り離して確実に動かせます。
...
ツール実行の前後に外部スクリプトを自動で呼ぶ仕組みです。

### 2. うれしさ

AI の返答を待たずに検証や通知を確実に動かせます。
...
保存時の自動テストや Slack 通知の仕込みに有効です。

### 5. はじめに

発火タイミング 4 種（PreToolUse / PostToolUse / UserPromptSubmit / Stop）を把握します。

### 6. 深掘り先

Tool Use（G-30）、settings.json（G-23）、Slash Command（G-32）。

## 開発フローでの位置（必須）

1. 設定を書く — `settings.json` の `hooks` 配列に発火タイミングとコマンドを登録します
2. ツールを呼ぶ — Claude Code がファイル保存などのツールを実行します
3. Hook が発火する — Pre/PostToolUse のタイミングでスクリプトが起動します
4. 結果を受け取る — 終了コードや出力が Claude Code に返されます
5. 次の処理へ — 終了コードに応じて続行かエラー扱いかが決まります
