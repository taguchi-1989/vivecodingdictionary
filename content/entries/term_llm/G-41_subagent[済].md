...
- 計算畑出身なのでサブルーチンから「エージェントの中のサブ」と理解しやすかったです。
- プログラミング経験がないと「1 本で全部やってしまおう」と捉えがちで、分ける理由が掴みにくいです。
- 「会社組織が作れるのでは？」と試したくなります。
...
- 👎 ダメな点: メンテが大変なときがあり、運用がずれたときの手当ても課題です。
- 👥 誰向けか: ハーネスを組んでアプリ作りをしたい人は押さえるべきです。
