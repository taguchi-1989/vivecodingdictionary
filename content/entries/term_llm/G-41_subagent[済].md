...
親が呼び出す役割特化の子エージェント。独立 Context で動き親を汚しません。

## 何をしてくれるか

`.claude/agents/<名前>.md` にプロンプトを書くと、親が Agent ツールで呼び出せます。結果だけを親に返すため、Context を汚さずタスクを分担できます。
...
- 自分は元々計算畑にいたので「サブルーチン（ルーチンの中の一部）」の発想から「エージェントの中のサブ」と理解しやすかった。
- 一方で、プログラミングに詳しくない人は「1 本で全部やってしまおう」と捉えがちで、なぜサブエージェントが必要なのかが計算系の経験がないと理解しづらい。
- これを使えば「会社組織が作れるのでは？」と思って試したくなる。

<!-- AUTHOR: user_only / AI-ASSIST: no -->
## 私のコメント

- 🙂 第一印象: すんなり入ってきた。
- 👍 良い点: 親タスクから委任することで、コンテキストの課題を相当解決できた。
- 👎 ダメな点: メンテが大変なときがある。「うまく使えているか」「もっと良い方法はないか」を見直すのが難しく、運用がずれていったときにどう手当てするかが課題。
- 👥 誰向けか: ある程度ハーネスを組んでモノづくりやアプリ作りをしたい人は押さえるべき。
