...
- 概念としては知っているが、Claude Code に「こうしたらこうして」と複数のルール（hook）を口頭で指示したとき、それが 100% 効いているのか分からない。
- モデルの追従性の問題なのか、単に登録がうまくいっていないのかも判別しづらい。
- 自分で細かく調整するときの手順が分かっておらず、口頭ベースだとつまずく。
...
- 👎 ダメな点: 細かい設定の仕方はやっぱりハードルが高く感じる。ざっくり Claude Code に任せがちになる。
- 👥 誰向けか: エージェントに作業を任せようと思ったときに、これが効かないと不安になる人。
