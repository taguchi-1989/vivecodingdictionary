...
親エージェントが呼び出す、役割特化の子エージェントです。独立した Context で動くので親の作業を汚しません。

## 何をしてくれるか

`.claude/agents/<名前>.md` にシステムプロンプトを書いておくと、親エージェントが Agent ツールでそのサブエージェントを呼び出せます。呼び出されたサブエージェントは独立した Context（コンテキスト）で動作し、結果だけを親に返します。

## どこで出会うか

Claude Code（略称 CC）を使った開発作業で「レビュー担当」「執筆担当」のように役割を分けたいときに登場します。本書プロジェクト自身も `.claude/agents/entry-writer.md` でサブエージェントを定義し、エントリ執筆を任せています。
...
「CC でレビューと執筆を分けるなら、Subagent に役割を特化させると Context が干渉しません。」
...
1. 役割を決める — 「reviewer」「writer」など、1 つのサブエージェントが担う機能を絞る
2. 定義ファイルを作る — `.claude/agents/<name>.md` にシステムプロンプトと使用ツールを書く
3. 親エージェントから呼び出す — Agent ツールでサブエージェント名と指示を渡す
4. 独立 Context で実行 — サブエージェントが作業し、結果を親エージェントに返す
5. 必要に応じて並列化 — 複数のサブエージェントを同時に起動してタスクを分担する
