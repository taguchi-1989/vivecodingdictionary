...
Context Window（コンテキストウィンドウ）は、LLM（大規模言語モデル）が 1 回の推論で参照できるトークン（Token）数の上限です。指示・会話履歴・添付ファイルはこの枠に収まる範囲でしか見えません。

## どこで出会うか

API のドキュメントやモデル比較表に「128K」「200K」「1M」と表記されます。長文を貼り付けて「入力が長すぎる」とエラーが出るのは、Context Window の上限を超えた場合です。
...
-お菓子はここがちっちゃくて、入れる時に分割したりとか色々工夫してたなでクロードコード使い始めた。排出の方はここがちっちゃくて、コンテキストの圧縮して文脈してなるってとかってのもよくあった
-
-
...
- 🙂 第一印象:覚えられる範囲がちっちゃいてのが課題なんだな
- 👍 良い点:最近のモデルはだいぶ改善してきているので、ここ大きくするのとハーネスによってボトルネックは解消しつつあるって感じかな
- 👎 ダメな点:全て入れようとする。プロセスは間違いだよね。だいぶ良くなったとはいえ、以前課題0になってないっていう感じ
- 👥 誰向けか:LL も使ってやる人はま必要だろうね。知識として
