...
System Prompt（指示書）・Tool Use（外部操作）・メモリ・プランニングを組み合わせ、LLM（大規模言語モデル）が複数ステップを自律実行する AI の構造を定義します。代表パターンに ReAct や Plan-and-Execute があります。

## どこで出会うか

Claude Code・Cursor・Devin などコーディングエージェントの裏側で使われる設計思想です。「なぜ AI が自分でコマンドを打てるのか」を知りたいとき行き着きます。
...
1. 目的定義 — タスクのスコープと自律度を決めます
2. 設計パターン選択 — ReAct / Plan-and-Execute などを用途に応じて選びます
3. ツール・メモリ設計 — Tool Use と短期・長期メモリの接続を組みます
4. ループ実装 — LangChain / AutoGen 等でエージェントループを構築します
5. 評価・改善 — Reflexion 的な自己評価で品質を上げます
...
-えっとここの言葉自体。今回初めて聞いたんだけど、ま重要な概念だなとは思う。オーケストレーションどうやって回すのかとかっていうことだとは思うんだけど、ちょっと会ってるかどうか確認してほしい。この辺もね。あのやろうとしてることは同じオーケストレーションとか turls の方法行ってるだけど、言葉が若干違うところがあってこの辺難しいなって思う
-
-
...
- 🙂 第一印象:今回初めて聞いたな
- 👍 良い点:この概念自体はやっぱ重要だなっても
- 👎 ダメな点:ここがノウハウになってるから何だろう？モンハンになってるし、組織によって必要なものが違ったりするから汎用的なものはあ。それピークならに任せればいいけど、組織てやっぱ自分たちで考えなきゃいけないから、ここの何だろうな？メタ的なところっていうのかな？やっぱ重要だけど、重要とか認識されてないとこ難しいな。この辺、ちょっと上の方と混ぜといて
- 👥 誰向けか:
