...
System Prompt・Tool Use・メモリ・プランニングを組み合わせ、LLM が複数ステップを自律実行する AI の構造を定義します。代表パターンに ReAct や Plan-and-Execute があります。
...
「Agent」の定義は業界で異なり、強化学習文脈とは別物です。

### 4. どこで役立つか

コーディングエージェントや業務自動化の構築時に役立ちます。
...
- 言葉自体は今回初めて聞いた。重要な概念だと思うが、オーケストレーションをどう回すかという話で合っているか確認したい。
- やろうとしていることは同じ（オーケストレーションやツールの組み合わせ）でも、言葉が微妙に違ってきて、この辺の使い分けが難しい。
- 組織ごとにノウハウ化していて汎用化しづらく、メタ的に重要な部分なのに重要と認識されにくい。

<!-- AUTHOR: user_only / AI-ASSIST: no -->
## 私のコメント

- 🙂 第一印象: 今回初めて聞いた。
- 👍 良い点: この概念自体はやっぱり重要だと思う。
- 👎 ダメな点: ノウハウ化していて組織ごとに正解が違うため汎用化しづらい。汎用的な部分はベースモデルに任せられるが、組織独自の設計は自分たちで考えるしかない。
- 👥 誰向けか:
