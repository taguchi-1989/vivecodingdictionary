...
System Prompt（システムプロンプト）は、ユーザーの発言より先に LLM（大規模言語モデル）へ渡す指示層です。役割や制約をここに書くことで、会話ごとに同じ指示を繰り返さずに済みます。

## どこで出会うか

Claude や ChatGPT の API ではリクエストの `system` フィールドがこれにあたります。ChatGPT の「カスタム指示」や Claude の「プロジェクト指示」も同じ仕組みです。バイブコーディングでは役割設定や出力形式の統一をここで固めます。
