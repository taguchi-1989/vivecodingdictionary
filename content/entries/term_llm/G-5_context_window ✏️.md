...
Context Window（コンテキストウィンドウ）は、LLM（大規模言語モデル）が 1 回の推論で参照できるトークン（Token）数の上限を示します。指示・会話履歴・添付ファイルのすべてがこの枠に収まる範囲でしか LLM には見えません。

## どこで出会うか

API のドキュメントやモデル比較表に「128K」「200K」「1M」のような表記で登場します。長いドキュメントを貼り付けたとき「入力が長すぎる」とエラーが出るのは、Context Window の上限を超えた場合です。
...
「Context Window が広いと長文を一度に渡せますが、lost in the middle には注意が必要ですよね。」
