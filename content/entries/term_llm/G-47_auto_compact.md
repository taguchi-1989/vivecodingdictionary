...
会話履歴がコンテキスト枠（Context Window）を超えそうになると、自動で要約・圧縮して続きを続けられる仕組みです。

<!-- ━━━━━━━━ 左ページ ━━━━━━━━ -->

## 何をしてくれるか

Claude Code（略称 CC）が内部で使用するトークン（文字量の単位）が一定割合（約 80%）に達すると、過去の会話履歴を短いダイジェストに置き換えます。圧縮後も会話を続けられるため、長い作業セッションが途切れません。

## どこで出会うか

CC を使って数時間〜数日にわたる開発セッションを続けると、画面に「会話が圧縮されました」のような通知が表示されます。`/compact` スラッシュコマンドで手動実行することも可能です。デフォルトは ON のため、意識しなくても自動で動きます。
...
「長いセッションでは Auto-compact を意識して、要点を CLAUDE.md にメモしながら進めます。」
