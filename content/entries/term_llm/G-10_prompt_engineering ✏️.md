...
Prompt Engineering（指示文の設計）は、LLM（大規模言語モデル）への入力を意図した出力が出るように整える技術です。役割・手順・制約を明示すれば、回答の正確さや再現性が上がりやすくなります。

## どこで出会うか

「プロンプトを工夫してね」と言われたときや、AI の回答が期待と外れたときに意識します。Claude や ChatGPT の利用ガイドで「効果的な聞き方」として紹介され、実務では指示の改善を繰り返す作業に現れます。
...
指示文の構造・語順・具体性を調整し、LLM の出力を意図した方向へ誘導します。

### 2. うれしさ

モデルを変えずに回答品質を上げられ、コスト抑制と精度改善を両立できます。

### 3. 注意点

対象は指示テキスト単体の設計で、文脈全体の設計は G-11 Context Engineering が扱います。

### 4. どこで役立つか

定型タスクの自動化や、チームで指示テンプレを共有する場面で効きます。
