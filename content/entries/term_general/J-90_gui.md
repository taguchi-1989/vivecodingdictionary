...
ウィンドウ・アイコン・メニュー・ボタンなどの視覚要素を通じて、コマンドを覚えなくても直感的にソフトウェアを操作できます。1984 年の Apple Macintosh 登場以降、PC 操作の標準形として定着しました。

## どこで出会うか

OS のデスクトップ画面や Web ブラウザ、アプリの設定画面はすべて GUI です。AI ツールの文脈では「Claude の Computer Use が GUI を自動操作する」といった形で登場し、AI が人間用の画面を読み取って操作する話題で頻出します。
...
AI が GUI を自動操作する Computer Use や RPA の文脈で重要です。

### 5. はじめに

GUI は「絵で操作」、CLI は「文字で操作」という対比で覚えると整理しやすいです。
