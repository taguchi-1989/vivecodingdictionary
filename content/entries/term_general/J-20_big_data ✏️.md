...
Volume（量）・Velocity（速度）・Variety（多様性）の 3V が自社システムで扱いきれない規模になったデータです。明確な閾値はなく、「これまでの環境では処理しにくい」という相対的な状態を指す概念です。

## どこで出会うか

2010 年代の DX（デジタルトランスフォーメーション）資料やデータ基盤のプレゼンで頻繁に登場します。機械学習（J-10）の学習データを語る文脈でも使われ、現在は RAG（G-15）やデータレイクハウスでも目にします。
