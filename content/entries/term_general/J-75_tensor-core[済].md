...
行列の掛け算（GEMM）を汎用コアより高速に処理します。FP16・BF16・FP8 など低ビット幅の混合精度演算に対応し、LLM の学習や推論で必要な大規模行列計算を短時間でこなせます。


## どこで出会うか

GPU スペック表や AI 学習環境の選定記事で「Tensor コア数 × 世代」として登場します。H100・RTX 40 シリーズ選びで TFLOPS を比較する場面が代表的で、世代が新しいほど対応精度のビット幅が広がる傾向があります。
...
- マニアックすぎてなかなか厳しい話題です。CUDA コアと Tensor コア、テンソル計算をしているのかな、というぼんやりしたイメージはあるものの、中でどう動いているか、行列計算がどう載っているのかをイメージしきれません
- 
- 
...
- 🙂 第一印象: 行列計算に特化したコアなのかな、という想像で入りました
- 👍 良い点: 世代を経るごとにどんどん強くなっており、LLM の性能向上やコスト低減に効いている点
- 👎 ダメな点: 正直よく分かりません。立ち位置を正しく説明できるか、というところで止まります
- 👥 誰向けか: 原理原則を理解したい人向け
