...
画像をトークン列に変換して LLM（大規模言語モデル）に渡し、「この図は何を示すか」「スクショのどこが崩れているか」をテキストで答えます。OCR 的な解析やグラフの数値読み取りにも使えます。

## どこで出会うか

ChatGPT や Claude に画像を貼ると背後で動いています。バイブコーディングでは UI のスクショを渡してレイアウト崩れを指摘させたり、Playwright MCP と組み合わせてブラウザのデバッグに使う場面で登場します。
