...
オープンソースの Unix 系 OS カーネルです。サーバ・AI 基盤・組込み機器の土台として広く使われています。

## 何をしてくれるか

CPU やメモリなどのハードウェアを管理し、アプリが動く土台を提供します。GNU ツールと組み合わせた「ディストリビューション（distro）」として配布されるのが一般的です。

## どこで出会うか

AI 学習・推論サーバはほぼ Linux 上で動いています。Docker や Kubernetes もこの OS を前提としており、クラウドインフラを調べると必ず名前が出ます。Windows 上は WSL（F-82）で触れることができます。
...
通常使ってる？パソコンが Windows ばかりなので、いきなりリラックスって言われる時にま何が変わってるのかどう変わるのかってのはわかんないってのが1つ2つ目がコマンドプロンプトとかパワーシェルとかじゃなくて場所になるのでまここの差分だよね。ここが非常にかなので、実はネイティブアプリが環境によって色々変わってるってところ、この辺が結構むずいよね。ターミナルも含めて
-
-
...
- 🙂 第一印象:なんで Windows 使わないんだろうな
- 👍 良い点:ただで使えるしめっちゃいいじゃん。ネイティブ的にはこっちの方が AI テントとも相性良さそう
- 👎 ダメな点:メジャー度かな？メジャードットづらさがあるよねねセキュリティのアップデートとか気になる
- 👥 誰向けか:エンジニア向けかな？まだまだ
