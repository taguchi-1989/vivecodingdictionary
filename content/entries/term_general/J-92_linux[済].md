...
CPU やメモリなどのハードウェアを管理しアプリが動く土台を提供します。GNU ツールと組み合わせた「ディストリビューション」として配布されます。

## どこで出会うか

AI 学習・推論サーバはほぼ Linux 上で動いています。Docker もこの OS を前提としており、クラウドインフラで必ず名前が出ます。
...
「Linux」はカーネルのみで、使うにはディストリビューションを選ぶ必要があります。

### 4. どこで役立つか

AI サーバ・クラウド・Docker など開発インフラのほぼ全域で必要です。
...
- 通常使うパソコンが Windows ばかりなので、いきなり Linux と言われても、何がどう変わるのかが分からない。
- コマンドプロンプトや PowerShell ではなく bash になるので、その差分がきつい。
- ネイティブアプリが環境によって色々変わるところも、ターミナル含め結構難しい。

<!-- AUTHOR: user_only / AI-ASSIST: no -->
## 私のコメント

- 🙂 第一印象: なんで Windows を使わないんだろうな、というところから。
- 👍 良い点: 無料で使えるしめっちゃいい。ネイティブ的にはこっちの方が AI エージェントとも相性が良さそう。
- 👎 ダメな点: メジャーじゃないところ。マイナー故のとっつきづらさやセキュリティ更新が気になる。
- 👥 誰向けか: まだエンジニア向けかな。
