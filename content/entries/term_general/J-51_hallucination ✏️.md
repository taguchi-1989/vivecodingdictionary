...
LLM がもっともらしい嘘をつく現象です。固有名詞・数値・URL で特に起きやすい限界です。

<!-- ━━━━━━━━ 左ページ ━━━━━━━━ -->

## 何をしてくれるか

Hallucination（幻覚）は、LLM が事実と異なる情報を自信満々に生成してしまう現象を指します。「次の単語を予測する」仕組みの副作用であり、LLM を使うすべての場面で意識しておく必要があります。

## どこで出会うか

「存在しない論文を引用された」「社名や日付が間違っていた」「URL を踏んだら 404 だった」といった場面で出会います。Context（文脈）が不足した質問ほど起きやすく、Web 検索や RAG（検索拡張生成）を組み合わせると軽減できることがあります。
...
「固有名詞・数値・URL は必ず一次情報で確認する」の一点から始められます。
...
1. 質問を組み立てる — Context が薄いと Hallucination が増えると意識します。
2. Context を整える — 公式資料・データを添えて LLM に渡します（G-1 参照）。
3. 回答を受け取る — 固有名詞・数値・URL は一次情報と照合します。
4. RAG を検討する — 検索結果を Context に組み込み、誤情報を減らします。
5. 運用で監視する — 本番利用では定期的に出力の事実確認を行います。
