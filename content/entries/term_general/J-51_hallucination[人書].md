...
「存在しない論文を引用された」「社名や日付が違う」「URL が 404 だった」といった場面で出会います。Context（文脈）が不足した質問ほど起きやすく、Web 検索や RAG（検索拡張生成）を組み合わせると軽減できます。
...
-ハーネスで留めないと、今の弱いaiだと信頼しきれないよね。ソースチェックなどさせないと機械的に。最後に人が見ないといけない。
-どんな人も傷えっとできそうと思って、騙されるのは変わんない。というか非常に問題だよね
-でも実はこれってハーネス。うまく作れてないっていう風な裏返して今の罪を取っていう風な。ところで、自分のリテラシーの無さっていうところに繋がってくるのも面白いところ
