...
MCP 公式の `mcp-server-sqlite` が、手元の `.db` ファイルへの SQL 実行・スキーマ参照などを提供します。コードを書かずに「先月の売上だけ集計して」と自然言語で指示できます。

## どこで出会うか

「Claude に DB を集計させたい」と思ったとき、MCP 公式ドキュメントや公式リポジトリで紹介されています。`uvx mcp-server-sqlite` の 1 行で使い始められます。
...
- SQL っていうのはデータベース系のところ名前そしてライトっていう風なとこ sqlite とか SQL ライトなのかとかとか分かりづらい。この2つかな
- 
- 
...
- 👍 良い点: ローカルで完結させるようなもので、データベース使う時には使うと叩いてくれつって便利そう。というか遠く効率が上がって
- 👎 ダメな点: ちょっとハードルあるよね？ってところとポストフでスキューエルとかっていうところ、その互換性のところがはてな
- 👥 誰向けか: ローカルで完結するような小規模のシステム作ってる人の第1弾かな？ポストグレードとか行く前かな
