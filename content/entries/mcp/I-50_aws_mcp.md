...
AWS が `awslabs/mcp` リポジトリで公開している MCP Server の総称です。CDK（クラウド開発キット）のスタック操作・CloudFormation テンプレートの検証・Bedrock Knowledge Base からの検索など、用途別に複数の Server が用意されています。

## どこで出会うか

Claude Code などの MCP 対応ツールで「インフラ作業を AI に手伝わせたい」場面で登場します。`uvx awslabs.aws-cdk-mcp-server@latest` のように Server ごとに起動し、必要なものだけ追加する構成が一般的です。
