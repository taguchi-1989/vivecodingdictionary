...
MCP Server（エムシーピーサーバー）を自作するときの出発点となるひな形です。公式 SDK が提供するボイラープレートから始めると、最小構成をすぐ動かせます。
...
TypeScript SDK の `create-mcp-server` コマンドや Python SDK の `mcp` パッケージが、ツール定義・リソース定義・プロンプト定義の 3 点セットをひな形として生成します。中身を埋めるだけで動作する Server が手に入ります。

## どこで出会うか

Anthropic 公式の `modelcontextprotocol/servers` リポジトリには Filesystem・GitHub・Slack などのリファレンス実装が並んでいます。自作 MCP の構造を学ぶ教材として、あるいはコピー元として参照する機会があります。
...
「create-mcp-server でひな形を出して、ツール定義だけ書き換えれば動きますよ。」
