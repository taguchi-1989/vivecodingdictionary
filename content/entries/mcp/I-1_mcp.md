...
LLM とツール（GitHub、Slack など）を共通の方式で接続する規格で、2024 年に Anthropic が提案しました。Server／Client／Transport の 3 役で構成され、一度作った MCP は対応するどの LLM からも使えます。

## どこで出会うか

Claude Code や Cursor の「拡張カタログ」規格です。MCP を 1 つ入れるだけで GitHub 操作や社内 DB 連携が実現でき、MCP 前提のエージェント設計が標準になります。
