...
日本語では「文脈」と訳されます。LLM に渡して考えさせる情報のすべて、という意味で使われます。

<!-- ━━━━━━━━ 左ページ ━━━━━━━━ -->

## ひとことで

Context（コンテキスト）は、日本語で言えば **「文脈」** です。LLM に入力として渡される情報のすべて — 指示、参考資料、過去のやり取り、ツールからの結果まで、モデルが「見て考える材料」がぜんぶここに入ります。

## 何をしてくれるか

LLM は Context に入っている情報だけを手がかりに応答します。記憶を持たないので、同じ LLM でも Context の中身が違えば応答が違います。

Context の典型的な構成は次のとおりです。

- **System Prompt**：役割や方針を最初に与える
- **指示（ユーザーの発話）**：今やってほしいこと
- **添付資料**：コード、ドキュメント、画像、PDF
- **会話履歴**：過去のやり取り
- **ツール結果**：検索結果、ファイル読み込み結果、コード実行結果

### 歴史的に一気に伸びた量

Context に入れられる上限（Context Window）は、GPT-4 Turbo の時代は 128K トークン程度でした。それが今では **100 万トークンクラス**（Gemini 1.5 など）まで広がり、数年で桁が動いています。長文をそのまま入れて考えさせる、という使い方が現実的になってきています。

ただし上限が大きくなっても、**数が増えすぎるとモデルが「バカになる」問題**は残ります。入れすぎた Context の中から必要な情報を見失う、という現象で、ベンチマークでは「Needle in a Haystack」のようなグラフで計測されます。
...
- Hallucination — Context が曖昧・不足・過剰なときに出やすい誤情報
...
膨らませすぎるとハルシネーションや応答の劣化を招きます。圧縮しすぎても情報が歪みます。
...
System Prompt ＋ 指示 ＋ 資料 ＋ 履歴 の 4 要素と、Context Window の上限の存在。

### 6. 深掘り先

Context Engineering、Prompt Caching、RAG、Memory、会話クリアの判断。
