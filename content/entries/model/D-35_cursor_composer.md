...
Cursor に搭載された複数ファイル横断の自動編集エージェントです。1 つの指示でリポジトリ全体に差分を出します。

<!-- ━━━━━━━━ 左ページ ━━━━━━━━ -->

## 何をしてくれるか

「〇〇を実装して」と指示すると、リポジトリ全体を参照しながら複数ファイルの差分を生成します。Apply ボタン 1 つで一括反映できるため、ファイルをまたぐリファクタも 1 往復で済みます。

## どこで出会うか

Cursor（B-4）の画面右側チャットを Composer モードに切り替えると使えます。バイブコーディングの解説記事で「Tab 補完とは別の機能」として紹介されることが多く、GitHub Copilot（B-5）との比較でも話題に出てきます。
