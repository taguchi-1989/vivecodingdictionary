...
文章生成・コード生成・画像理解・ツール呼び出しを担うモデル群です。2024 年中盤に公開された Sonnet 3.5 でコーディング適性が大きく向上し、Artifacts（成果物プレビュー）や Computer use（画面操作自動化）もこの世代で先行公開されました。

## どこで出会うか

現在は 4 系が主力のため、過去の記事やチュートリアルで「Sonnet 3.5 で試した」という記述を見かける場面が出会いの入口です。どの版・ティアが使われていたかを把握すると、再現性の確認や 4 系への移行比較に役立ちます。
