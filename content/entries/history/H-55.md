...
LLaMA（Large Language Model Meta AI）は、Meta が研究・商用向けに公開した大規模言語モデル群です。誰でもモデルの重みをダウンロードして手元で動かせる状態にしたことで、自前サーバーや個人 PC での推論が現実的になりました。

## どこで出会うか

「ローカル LLM を動かす」という文脈で必ず出てくる名前です。Ollama や LM Studio のセットアップ記事でモデル選択肢として登場し、Hugging Face のモデルページや Llama 3 系の記事を読む場面で名前を見かけます。
...
1. 2023-02 LLaMA 公開 — 研究者限定配布。まもなくファイルがリークし個人勢が注目。
2. 2023-07 LLaMA 2 正式公開 — Microsoft 共催で商用利用 OK のライセンスに。
3. ローカル推論が普及 — llama.cpp・Ollama・LM Studio が相次いで登場した。
4. 2024〜2025 後続モデル登場 — Llama 3 系・Mistral・Qwen・Gemma などが追随。
5. エコシステム拡大 — Hugging Face でのモデル共有文化が一気に加速した。
