...
2017 年に公開された "Attention Is All You Need"。現代 LLM（大規模言語モデル）の起点となった論文です。

<!-- ━━━━━━━━ 左ページ ━━━━━━━━ -->

## 何をしてくれるか

Google Brain の 8 名が書いたこの論文が、Attention（注意機構）だけで文章を処理する Transformer アーキテクチャ（設計の骨格）を提案しました。翌年には BERT・GPT-1 が登場し、その後の LLM ブームすべての基盤になっています。

## どこで出会うか

「現代 AI の起点」として技術記事や書籍で必ず参照されます。ChatGPT や Claude を説明する文脈で「Transformer ベース」という言葉を見かけたとき、この論文が土台にあります。
...
「Transformer 論文が 2017 年の起点で、BERT や GPT もそこから派生しているんですよね。」
...
1. 論文公開（2017-06）— Attention だけの新アーキテクチャを提案しました。
2. BERT / GPT-1（2018 年）— 事前学習モデルへ応用され、精度が跳ね上がりました。
3. GPT-3（2020 年）— 大規模化により文章生成の質が商用レベルに達しました。
4. ChatGPT（2022 年）— 一般公開で LLM が社会に浸透しました。
5. LLM 全盛（2023 年〜）— Claude・Gemini など多数のモデルが Transformer を基盤にしています。
