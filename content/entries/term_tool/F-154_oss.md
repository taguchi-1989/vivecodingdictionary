...
ライセンス条件を守ることで、誰でもソースコードを入手・改変・再配布できます。MIT / Apache 2.0（F-151）/ GPL（F-152）などのライセンスが条件を定め、商用利用も原則として可能です。


## どこで出会うか

GitHub で公開されているライブラリを npm や pip でインストールするとき、ほぼ必ず OSS に触れています。バイブコーディングでよく使う Llama / Mistral / Qwen などのローカル LLM（大規模言語モデル）もオープンウェイトで配布されており、OSS の仕組みが基盤にあります。
