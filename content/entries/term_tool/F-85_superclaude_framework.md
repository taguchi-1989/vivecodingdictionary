...
「SuperClaude 入れたら `/architect` で設計レビューを Claude Code に任せられるようになりました。」
...
1. インストール — `pip install superclaude` 等で `.claude/` にファイルを展開する
2. ペルソナ選択 — architect / frontend / qa など工程に合うペルソナを指定する
3. コマンド実行 — `/architect` `/refactor` など専用コマンドで Claude に作業を依頼する
4. 出力確認 — 生成されたコード・レビュー・ドキュメントをプロジェクトに取り込む
