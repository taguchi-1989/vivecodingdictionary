...
- AI の出力に `#` や `※` などの記号が混じってくるとどれが Markdown の記法なのか分かりにくいです。
- テキストで書いてあるので内容自体は読みやすいのですが、機能（記法）を理解するのに少し時間がかかりました。
- どこまでが標準の Markdown 記法でどこから拡張なのかが曖昧な場面があります。

<!-- AUTHOR: user_only / AI-ASSIST: no -->
## 私のコメント

- 🙂 第一印象: 何のために使うのかよく分からず、`#` などの記法が少し使いにくそうな印象でした。
- 👍 良い点: AI エージェントとの相性が非常に良く、人間にも読みやすいです。Mermaid や YAML など他の記法も埋め込める点も優秀です。
- 👎 ダメな点: 基本的にはないと思っています。
