...
HyperText Markup Language（ハイパーテキストマークアップランゲージ）の略。Web ページの骨格を作るマークアップ言語です。

<!-- ━━━━━━━━ 左ページ ━━━━━━━━ -->

## 何をしてくれるか

テキストや画像・リンクを「タグ（tag）」で囲み、ブラウザが読める構造に変換するマークアップ言語です。`<h1>` で見出し、`<p>` で段落、`<img>` で画像を意味し、ブラウザはその構造にしたがってページを表示します。

## どこで出会うか

AI にランディングページや簡単な Web フォームを作ってもらうと、出力の中に HTML ファイルが含まれます。ブラウザの「ページのソースを表示」を開いたときに並ぶタグの羅列が HTML です。VS Code（ビジュアルスタジオコード）でも `.html` 拡張子のファイルとして目にします。
