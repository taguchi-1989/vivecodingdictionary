...
リポジトリのルートに `.gitignore` を置くと、記載パターンに一致するファイルやフォルダを git の管理対象から除外します。ビルド成果物（`dist/`）・依存ライブラリ（`node_modules/`）・秘密情報（`.env`）が代表例です。

## どこで出会うか

プロジェクト開始時や GitHub のリポジトリ作成時に自動生成されます。AI エディタで作業する際も、`.gitignore` が整っていないと AI が `node_modules/` を誤って読み込むことがあります。
