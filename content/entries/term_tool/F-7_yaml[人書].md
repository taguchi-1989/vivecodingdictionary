...
階層構造のデータをインデントと記号で表し、設定ファイルとして使える形式です。JSON（ジェイソン）と同じデータを書けますが、コメントが書けて記号が少ないぶん、人が編集する用途に向きます。

## どこで出会うか

GitHub Actions（ギットハブアクションズ）のワークフロー定義、Docker Compose（ドッカーコンポーズ）の設定、本書のエントリ先頭の YAML フロントマターなどで目にします。AI ツールの設定ファイルにも使われます。
...
1. 設定ファイルの確認 — GitHub Actions や Docker Compose の `.yml` を開く
2. AI に修正を依頼 — 「この YAML に設定を追加して」と AI へ渡す
3. インデント検証 — 返ってきた YAML のインデントを目視または Lint で確かめる
4. 動作確認 — ワークフロー実行・コンテナ起動で設定が反映されたか確かめる
...
-名前読みづらいってのは1つとあのマークダウンファイル中で一緒に書いちゃうことが多いから、拡張子としてほとんど見ないっていうところ、たまに見る時に結構びっくりするっていうのとは、爆弾ファイル。なんかある時にどこからどこまでってのがこれもルールで決まってるけど、これ知らないと特に
-
-
...
- 🙂 第一印象:ファイルの一部って感じまマークダウン
- 👍 良い点:人の角性が非常に高いし、 AI の家族でもそこそこある感じのところ
- 👎 ダメな点:ジェイソンとかに比べると肩定期のところは完璧じゃないよね
- 👥 誰向けか:特に ま
