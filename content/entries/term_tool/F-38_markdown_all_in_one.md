...
VS Code で Markdown 編集を快適にする定番の拡張機能です。ショートカットや目次生成など編集補助を一括で追加します。

<!-- ━━━━━━━━ 左ページ ━━━━━━━━ -->

## 何をしてくれるか

VS Code（F-30）に Markdown（F-6）の編集補助機能を一気に追加します。Ctrl+B で太字、Ctrl+I で斜体、Enterキーで箇条書き継続、目次（TOC）の自動生成、チェックボックスの操作、テーブルの整形など、頻繁に使う操作をまとめて提供します。

## どこで出会うか

VS Code の拡張機能一覧で「Markdown」と検索したときに上位に出てきます。README や設計ドキュメントを書く場面、あるいはバイブコーディングで AI が生成したマークダウンを整形・編集する場面でこの拡張の名前を目にすることがあります。
