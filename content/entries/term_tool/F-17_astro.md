...
コンテンツ重視の Web フレームワークです。デフォルト静的で、必要な部分だけ JavaScript を載せます。

<!-- ━━━━━━━━ 左ページ ━━━━━━━━ -->

## 何をしてくれるか

HTML・Markdown・MDX を中心に書き、必要な箇所だけ React や Vue などのコンポーネントを「島（アイランド）」として配置できます。デフォルトで静的出力（SSG）なので初期ロードが軽く、ブログ・ドキュメント・LP に向いています。

## どこで出会うか

Claude や Cursor に「コンテンツ中心の静的サイトを作って」と頼むと、提案されることがあります。Next.js（F-11）が動的アプリ中心なのに対し、Astro はコンテンツ配信を優先したい場面で名前が出ます。Tailwind CSS（F-16）や shadcn/ui（F-15）との組み合わせも多いです。
