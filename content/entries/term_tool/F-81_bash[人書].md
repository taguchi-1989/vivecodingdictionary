...
ターミナル（端末）でコマンドを受け付け、ファイル操作・環境変数・パイプ（`|`）やリダイレクト（`>`）を組み合わせて実行します。シェルスクリプトにまとめれば、反復作業を自動化できます。

## どこで出会うか

Claude Code や Cursor の Bash ツールが裏で呼ぶのがこのシェルです。AI が出すコマンドはほぼ bash 互換で、ターミナルを開けばそこにあります。Windows では WSL（F-82）経由で使うことがあります。
