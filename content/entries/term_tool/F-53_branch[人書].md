...
- オフィス仕事では出てこない概念なので、最初の理解が難しかったです。ブランチ名を自分でつけるのも面倒で、適当にやっていると main と競合してブランチを丸ごと捨てる羽目になることが何度かありました。
- 最近は「こういうことをしたい」と AI に伝えると適切なブランチ名をつけてくれるので楽になりました。

<!-- AUTHOR: user_only / AI-ASSIST: no -->
## 私のコメント

- 🙂 第一印象: 概念が理解しにくいな、というのが最初の印象でした。
- 👍 良い点: 並列作業ができ、商用版と開発版を分けて持てるのは今では手放せない機能です。
- 👎 ダメな点: 必要性を理解するためにそれなりのリテラシーが前提として必要です。
