...
React 用の UI コンポーネント集です。npm 配布ではなく CLI でコードを直接コピーする方式が特徴です。

<!-- ━━━━━━━━ 左ページ ━━━━━━━━ -->

## 何をしてくれるか

Radix UI（アクセシブルな未スタイル化プリミティブ）と Tailwind CSS（F-16）を組み合わせたコンポーネントを提供します。`npx shadcn@latest add button` の 1 コマンドでコードがプロジェクトにコピーされ、自由にカスタマイズできます。

## どこで出会うか

Next.js（F-11）や Astro（F-17）で UI を組み立てる場面で登場します。AI アシスタント（Claude・GPT 等）に「shadcn/ui で作って」と伝えると素直に従ってくれるため、バイブコーディングでも定番の指定です。
