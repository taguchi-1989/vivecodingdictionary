...
作業ブランチを本流に取り込んでもらう依頼です。コードレビューの中心の場になります。

<!-- ━━━━━━━━ 左ページ ━━━━━━━━ -->

## 何をしてくれるか

作業した branch（ブランチ）の差分を、main などの本流へ統合してもらう依頼の仕組みです。タイトル・本文・差分・CI（継続的インテグレーション）の結果・レビュアーのコメントが 1 画面に集まります。

## どこで出会うか

GitHub（F-60）や GitLab などのホスティングサービスで、リポジトリの「Pull requests」タブから作成します。チーム開発では承認フローの入口になり、1 人開発でも PR を切ると変更履歴が残ります。
...
1. ブランチ作成 — git（F-50）で feature ブランチを切ります
2. コミット & プッシュ — 変更を commit してリモートへ push します
3. PR 作成 — Why / What / テスト計画を本文に書いて依頼します
4. レビュー & CI — コメントと GitHub Actions（F-62）の自動チェックを通します
5. マージ — 承認条件を満たしたら main へ取り込んで完了です
