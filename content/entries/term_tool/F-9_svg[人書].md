...
- 画像データのつもりで触ったら実はテキスト形式で、エディタで開くと数値の羅列が出てきて驚きました。
- できることの多さに期待が膨らむ一方、実際に試すと期待値より難しい場面がある、という体験が続いています。

<!-- AUTHOR: user_only / AI-ASSIST: no -->
## 私のコメント

- 🙂 第一印象: 軽いのにしっかりした画像が出せるのが良いと感じました。LLM に書かせることができると知って驚きました。画像なのに数値で定義されているという仕組みにも驚きがあります。
- 👍 良い点: LLM が得意な分野なので、絵を描かせると再現性があり、後から編集もできる点が優れています。
- 👎 ダメな点: LLM の学習がまだ完璧ではなく、CAD 相当のことができるレベルには至っていないのが惜しいです。
- 👥 誰向けか: 画像生成ではなく編集可能なものを作りたい人向けです。今後の可能性が広がる分野なのでウォッチしていく価値があります。
