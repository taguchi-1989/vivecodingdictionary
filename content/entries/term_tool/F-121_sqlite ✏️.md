...
ライブラリとして直接ファイルを読み書きするため、デーモン（常駐プロセス）の起動が不要です。SQL でデータを操作しながら、セットアップはファイルを 1 つ用意するだけで完結します。

## どこで出会うか

iOS・Android のアプリや Chrome・Firefox の内部でデフォルト採用されています。ローカル開発で「サーバを立てずに DB を試したい」場面でも頻繁に選ばれます。SQLite MCP（I-41）を使うと AI が直接 SQL を実行する事例も増えています。
