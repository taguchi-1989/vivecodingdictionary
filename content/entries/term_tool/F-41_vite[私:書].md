...
フロントエンドの開発サーバ起動とコードのビルドを担うツールです。変更時は該当モジュールだけを差し替える HMR（Hot Module Replacement）が動き、画面反映がほぼ瞬時です。

## どこで出会うか

React・Vue・Svelte などで npm（Node Package Manager）経由でインストールします。`npm run dev` で開発サーバが立ち上がる場面や、Bolt.new が内部で使う場面で出会います。
