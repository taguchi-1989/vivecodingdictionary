...
Agile（アジャイル）は 2001 年の「アジャイルソフトウェア開発宣言」で示された価値観で、短い反復・変化への追従・対話の重視が柱です。Scrum（スクラム）はその代表的な実装の型で、1〜4 週間の Sprint（スプリント）単位で動くものを届ける枠組みを指します。

## どこで出会うか

チームの朝会（Daily Stand-up）や Sprint Review の案内、あるいは Backlog（バックログ）管理ツールの画面で目にすることが多いです。AI ツール導入の場面でも「まず 1 Sprint 試して Retrospective（振り返り）で評価する」という使い方と相性が良く、近年よく組み合わされます。
...
「Sprint 単位で AI ツールを試して、Retrospective で振り返ると現場の納得感が高いです。」
