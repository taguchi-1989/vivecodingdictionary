...
`main` と `develop` を常設ブランチとして保持し、`feature/*`・`release/*`・`hotfix/*` の短命ブランチを目的別に切り分けます。役割が明確なため、複数人での並行開発が整理しやすくなります。

## どこで出会うか

チーム開発のオンボーディング資料やブランチ命名規則の説明で目にすることが多いです。「うちは Git Flow を採用している」と言われたら、このモデルに従ったブランチ管理が行われていることを指します。
...
- 🙂 第一印象: 初めて聞きました。
- 👍 良い点: やり方の形としては綺麗だと思います。
- 👎 ダメな点: 認知コストが高くなって運用が難しくなりますよね。
- 👥 誰向けか: わかりません。
