...
タグは必ず 1 個だけ。誰の番か × 何の作業かが瞬時に読めるよう、
「主体＋作業」を 2 文字で表す（2026-05-04 採用、2026-05-04 改：コロン廃止）。

  [人未]    skeleton / candidate
            → 人の番。AI に初稿生成を依頼する
  [AI直]    drafting + ☆ 違反あり
            → AI の番。構造崩れ・字数 100 字超などを直す
  [AI整]    drafting + 警告のみ（または全パスなのに昇格漏れ）
...
例:
  ...
  C-5_xai[drafting][comment].md → C-5_xai[AI整].md  （警告のみの場合）
  C-50_sam_altman[needs_review].md → C-50_sam_altman[人書].md
...
なぜ絵文字でもコロンでもなくテキストか:
  Obsidian Mobile + Git の同期で、絵文字付きファイル名は安定しなかった
  （古いキャッシュが上書きで戻る事故）。さらに「[人:書]」のように
  コロン入りにしたところ、Windows 系・iCloud/OneDrive 経由で同期したときに
  コロン入りファイルだけが消える事故が発生（[済]/[凍] は無事）。
  そこで コロンも含めず、半角英字＋漢字の 2 文字 [...] 形式に統一した。
  ファイル名安全文字（ASCII letters と一部の漢字）だけで構成しておけば、
  Win/Mac/Linux/Android/iOS いずれでも壊れない。
"""
...
# 全タグ（剥がし対象）。旧コロン入り・旧「私:〜」も剥がせるよう残す。
NEW_TAGS = [
    # 現行（コロンなし）
    "[人未]", "[AI直]", "[AI整]", "[人書]", "[済]", "[凍]",
    # 旧コロン入り（2026-05-04 同日中にコロン廃止）
    "[人:未]", "[AI:直]", "[AI:整]", "[人:書]",
    # 旧「私:〜」表記（2026-05-04 一日のみ存在）
    "[私:未]", "[私:書]",
]
