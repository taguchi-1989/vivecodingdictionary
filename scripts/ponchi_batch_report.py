...
def main() -> int:
    ...
    lines.append("- `color_audit` means a non-brand base candidate is staged and has passed the mechanical color gate; it still needs visual review before final promotion.")
    lines.append("- Generate or update scene briefs only for rows that truly lack prompts or need a rerun.")
    ...
    lines.append("python scripts\\ponchi_color_audit.py --manifest-root assets\\ponchi\\final_candidates")
    lines.append("```")
    lines.append("")
