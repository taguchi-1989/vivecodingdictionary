...
毎回の生成では、プロンプト文だけでなく次の PNG を視覚参照として使う。これらは現在の生成済みポンチ絵から切り出した基準画像。人物やロボットを出す場合は、この 3 ファイルの髪型、服、頭身、線幅、ロボット形状を基準にし、新しい別キャラや別マスコットを作らない。

| character | visual reference |
| :-- | :-- |
| Character A | `assets/ponchi/references/character-a-reader-woman.png` |
| Character B | `assets/ponchi/references/character-b-teacher-man.png` |
| Character C | `assets/ponchi/references/character-c-pet-robot.png` |
...
基本キャラは Character A/B/C の 3 名。ただし、多人数の会議、レビュー列、チーム作業、ユーザー群、座席が並ぶ場面など、人数そのものが意味を持つ構図では一時キャラを追加してよい。

一時キャラのルール:

- A/B/C を置き換えない。主役、説明役、操作役は原則 A/B/C のどれかにする。
- 一時キャラは背景、脇役、群衆、レビュー参加者、利用者として扱う。
- 顔、服、髪型は A/B/C より簡略化し、個性を強くしない。
- 実在人物、著名人、職業コスプレ、ブランド服にしない。
- 次の画像に固定キャラとして持ち越さない。その場限りのテンポラリー扱いにする。
- 多人数を出しても手や指の描写を増やしすぎない。細かい操作は主役 1 人に集約する。
