...
| `ponchi-batch-002` | 20 / 20 | 20 / 20 | base complete; 18 overlays in `overlay_audit`, 0 remain `overlay_wait`; B-19 and B-23 are confirmed logo-less in `color_audit` |
| `ponchi-batch-003` | 20 / 20 | 20 / 20 | base complete; 19 overlays in `overlay_audit`, 0 remain `overlay_wait`; C-8 is confirmed logo-less in `color_audit` |
| `ponchi-batch-004` | 20 / 20 | 20 / 20 | base complete; 8 overlays in `overlay_audit`, 0 remain `overlay_wait`, 12 are `logo_avoid`; C-12 is confirmed logo-less in `color_audit` |
...
| non-brand base candidates in `color_audit` | 217 |
| review-pending final candidates staged | 350 |
| remaining `overlay_wait` rows | 0 |
...
All 350 staged final candidates now pass the mechanical color gate. This
includes 133 official-overlay candidates and 217 non-brand base candidates.
...
before restaging. B-23 AWS, C-8 Microsoft AI, and C-12 TSMC were moved into the
same logo-less lane after official source review found that a deterministic
logo overlay would be weaker than a no-logo candidate under the no-fake-logo
rule.
...
- `ponchi-batch-004`: C-10 Moonshot AI, C-11 Z.ai, C-13 Groq, C-14 AMD, C-80 AI大学, C-81 にゃんた, C-82 まさお, and C-83 AI の羅針盤 deterministic official overlays staged as review-pending candidates. C-12 TSMC is staged as a logo-less base candidate because no usable official TSMC logo asset was confirmed in this environment. C-80 - C-83 use official YouTube channel avatars as official icon overlays. C-83's source page displays `AI時代の羅針盤`, while the ledger title remains `AI の羅針盤`; keep that naming mismatch as a final-review note. C-10 still shows `review` in the Batch 004 overlay image audit because its clearspace ink is high; do not promote it without visual review.
