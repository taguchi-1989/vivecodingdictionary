...
A-1〜A-11 の 11 エントリを、扉 1 ＋ 本付け 6 の **7 見開き** に再編成します。ID は維持し、複数 ID を 1 見開きに同居させるときは **同じ `page_layout` 値を共有させる**ことで束ねます（`spread_group` 別フィールドは設けない。§4-2）。

| # | 構成 ID | 見開き名 | layout 名 |
| :-: | :-- | :-- | :-- |
| 0 | （ID `front_concept`・ledger 外） | 扉「知らないことばで、止まらない。」 | `front_concept_spread` |
