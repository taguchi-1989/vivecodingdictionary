...
クラスタ単位と pass mode は `docs/ponchi_semantic_audit/semantic_cluster_registry.md` を参照する。特にモデル世代系は、画像だけで exact entry を当てることを常に要求しない。`family`, `era`, `capability_milestone`, `model_type`, `deployment_context` のどれを判定対象にするかを先に決める。
...
- 根拠の弱い世代差や minor version 差は、`honest_ambiguous` として本文・キャプション補助に回す。
- 未確認の内部アーキテクチャや能力を絵で断定しない。
